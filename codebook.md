...
A data.table named `tidy.data` is set with the following columns with all units maintained from the original format.
...
A data.table named `tidy` is set with the following columns.  All units are maintained from the original data set. A file named tidy.txt is created from run_analysis.R.
